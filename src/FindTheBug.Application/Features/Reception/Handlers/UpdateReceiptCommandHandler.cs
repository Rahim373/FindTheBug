...
public class UpdateReceiptCommandHandler(IUnitOfWork unitOfWork)
    : ICommandHandler<UpdateReceiptCommand, ReceiptResponseDto>
{
    public async Task<ErrorOr<Result<ReceiptResponseDto>>> Handle(UpdateReceiptCommand request, CancellationToken cancellationToken)
    {
        ...
        // Check if invoice number already exists for different receipt
        var existing = await unitOfWork.Repository<LabReceipt>().GetQueryable()
            .FirstOrDefaultAsync(r => r.InvoiceNumber == request.InvoiceNumber && r.Id != request.Id, cancellationToken);
        ...
        receipt.InvoiceNumber = request.InvoiceNumber;
        receipt.FullName = request.FullName;
        receipt.Age = request.Age;
        receipt.IsAgeYear = request.IsAgeYear;
        receipt.Gender = request.Gender;
        receipt.PhoneNumber = request.PhoneNumber;
        receipt.Address = request.Address;
        receipt.ReferredByDoctorId = request.ReferredByDoctorId;
        receipt.SubTotal = request.SubTotal;
        receipt.Total = request.Total;
        receipt.Discount = request.Discount;
        receipt.Due = request.Due;
        receipt.Balace = request.Balance;
        receipt.ReportDeliveredOn = request.ReportDeliveredOn;
        receipt.LabReceiptStatus = request.LabReceiptStatus;
        receipt.ReportDeliveryStatus = request.ReportDeliveryStatus;

        // Remove existing test entries
        foreach (var testEntry in receipt.TestEntries)
        {
        ...
        await unitOfWork.Repository<LabReceipt>().UpdateAsync(receipt, cancellationToken);

        // Get the updated receipt with related data
